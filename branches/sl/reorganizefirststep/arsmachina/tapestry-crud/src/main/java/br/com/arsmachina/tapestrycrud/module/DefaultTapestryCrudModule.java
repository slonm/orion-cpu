...
public class DefaultTapestryCrudModule implements TapestryCrudModule {
     ...
     * @return a {@link String} or null.
     */
    public String getSingleTypeSelectModelFactoryClassName(Class<?> entityClass) {

        return String.format("%s.selectmodel.%sSelectModelFactory", getTapestryPackage(),
                entityClass.getSimpleName());
    ...
    public <T> Class<? extends SingleTypeSelectModelFactory<T>> getSingleTypeSelectModelFactory(Class<T> entityClass) {
        return getClass(getSingleTypeSelectModelFactoryClassName(entityClass));
    }

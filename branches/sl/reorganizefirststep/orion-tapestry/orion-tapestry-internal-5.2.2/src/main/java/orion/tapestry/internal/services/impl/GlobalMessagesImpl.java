...
public class GlobalMessagesImpl implements MethodAdvice, GlobalMessages {
    ...
    private final Map<Locale, List<Messages>> messagesListByLocale = new HashMap<Locale, List<Messages>>();
    ...
    public GlobalMessagesImpl(List<String> configuration,
        ...
        messagesSource = new MessagesSourceImpl(new URLChangeTracker(classpathURLConverter), parser);
    }

    private List<Messages> getMessagesList(Locale locale) {
        if (!messagesListByLocale.containsKey(locale)) {
            List<Messages> list = new LinkedList<Messages>();
            for (Resource r : resources) {
                list.add(messagesSource.getMessages(new InternalBundle(r), locale));
            }
            messagesListByLocale.put(locale, list);
        }
        return messagesListByLocale.get(locale);
    }

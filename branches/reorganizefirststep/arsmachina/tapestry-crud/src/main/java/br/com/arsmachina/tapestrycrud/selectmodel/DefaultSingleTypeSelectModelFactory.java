...
public class DefaultSingleTypeSelectModelFactory<T> implements SingleTypeSelectModelFactory<T> {
	...
	public DefaultSingleTypeSelectModelFactory(Controller<T, ?> controller,
		...
		if (encoder == null) {
			throw new IllegalArgumentException("Parameter encoder cannot be null");
        }
    ...
    public SelectModel create(List<T> objects) {
        ...
        for (T object : objects) {
            ...
            final String label = encoder.toLabel(object);
    ...
    public <V> SelectModel create(Class<V> propertyType, String propertyName,
            SingleTypeSelectModelFactory<V> propertySelectModelFactory) {
        return propertySelectModelFactory.create(null);
    }

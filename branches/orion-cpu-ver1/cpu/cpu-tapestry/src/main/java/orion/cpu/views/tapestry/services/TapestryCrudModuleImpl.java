...
public class TapestryCrudModuleImpl extends DefaultTapestryCrudModule {
    ...
    protected String getPrimaryKeyEncoderClassName(Class<?> entityClass) {
        return getClassName(entityClass, "orion.primarykeyencoder-package", "PrimaryKeyEncoder");
    }
    ...
    public String getEditPageURL(Class<?> entityClass) {
        if (Edit.class.getName().equals(getEditPageClassName(entityClass))) {
            return Edit.class.getSimpleName();
        }
    ...
    public String getListPageURL(Class<?> entityClass) {
        if (ListView.class.getName().equals(getListPageClassName(entityClass))) {
            return ListView.class.getSimpleName();
        }
    ...
    public String getViewPageURL(Class<?> entityClass) {
        if (ListView.class.getName().equals(getViewPageClassName(entityClass))) {
            return ListView.class.getSimpleName();
        }
     ...
     * @return a {@link String}.
     */
    //TODO может сделать /sec/ListUser вместо /sec/User/List?
    private String getURL(Class<?> entityClass, final String prefix) {
        final String className = entityClass.getSimpleName().toLowerCase();
        final String schema = BaseEntity.getSchema(entityClass);
        return String.format("%s%s%s", schema.length() != 0 ? schema + "/" : "",
                prefix, className);
    }
